...
def process_product_second_price_step(message, product_name, product_image, product_first_price):
    ...
    msg = bot.reply_to(message, "🍽 Mahsulot turini tanlang:" if user_data[str(message.chat.id)]["language"] == "🇺🇿 O'zbek" else "🍽 Выберите категорию продукта:", reply_markup=types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True).add(
        types.KeyboardButton("🥗 Birinchi ovqat"), types.KeyboardButton("🍛 Ikkinchi ovqat"), types.KeyboardButton("🥗 Salat"), types.KeyboardButton("🍱 Setlar"), types.KeyboardButton("❌ Orqaga")
    ))
...
def process_product_category_step(message, product_name, product_image, product_first_price, product_second_price):
    ...
    conn.commit()
    second_price_info = f" - {product_second_price} so'm" if product_second_price else ""
    bot.reply_to(message, f"✅ Mahsulot qo'shildi: {product_name} - {product_first_price} so'm{second_price_info}" if user_data[str(message.chat.id)]["language"] == "🇺🇿 O'zbek" else f"✅ Продукт добавлен: {product_name} - {product_first_price} сум{second_price_info}", reply_markup=create_admin_menu())
...
def show_category_items(message):
    ...
    if not items:
        bot.reply_to(message, "❌ Bu bo'limda hech narsa yo'q." if user_data[str(message.chat.id)]["language"] == "🇺🇿 O'zbek" else "❌ В этой категории пока ничего нет.", reply_markup=create_main_menu(user_data[str(message.chat.id)]["language"]))
        return
...
def clear_cart(message):
    ...
    cursor.execute("DELETE FROM cart WHERE user_id = ?", (user_id,))
    conn.commit()
    bot.reply_to(message, "🗑️ Savatcha tozalandi." if user_data[str(message.chat.id)]["language"] == "🇺🇿 O'zbek" else "🗑️ Корзина очищена.", reply_markup=create_main_menu(user_data[str(message.chat.id)]["language"]))
